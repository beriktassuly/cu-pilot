...
try {
  ...
  for await(const line of createInterface({input:process.stdin})){
    // The native event channel is bounded; consume it even when tests do not use logs.
    surf.drainEvents();
    const command=JSON.parse(line);if(command.action==='stop')break;
    if(command.action==='next'){
      // Real local tick execution advances the bank. No synthetic observation slots.
      const {value:tickLifetime}=await rpc.getLatestBlockhash().send();
      const tick=buildTransferBatch({version:'legacy',payer:kit.address(surf.payer),destinations:[FIXTURE_RECIPIENT],amounts:[BigInt(command.slot)+1n],blockhash:tickLifetime.blockhash,lastValidBlockHeight:tickLifetime.lastValidBlockHeight});
      const signedTick=await kit.signTransaction([key],kit.compileTransaction(tick));
      await rpc.sendTransaction(kit.getBase64EncodedWireTransaction(signedTick),{encoding:'base64',skipPreflight:false,preflightCommitment:'confirmed'}).send();
      ...
      const nextBound=bindMessage(next);
      console.log(canonical({current_slot:slot.toString(),serialized_base64:nextBound.wireBase64,builder_preparation_ms:performance.now()-preparationStarted,builder_state_reads:2}));continue;
    }
