"""Real offline loader-v3 Upgrade followed by deployment suspension and fallback.

Run after npm ci in typescript and tests/integration/runtime:
    uv run python tests/integration/program_upgrade.py
On Windows set CU_PILOT_UPGRADE_NODE to the full JSON WSL launch command.
...
def main() -> None:
    ...
    command = json.loads(os.environ.get("CU_PILOT_UPGRADE_NODE", '["node"]'))
    if not os.environ.get("CU_PILOT_UPGRADE_NODE"):
        command.append(str(root / "tests/integration/upgrade.mjs"))
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="cu-pilot-upgrade-") as directory:
            with RpcClient(ready["rpc_url"], requests_per_second=100) as rpc:
                ...
                before = refresh_deployments(
                    registry,
                    AtomicAccountReader(
                        rpc, [*programs, ready["program_data"]], context.current_slot
                    ),
                    programs,
