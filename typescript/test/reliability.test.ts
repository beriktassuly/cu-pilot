...
import { loadArtifact, predictResources, roundedLimit, slot, wilsonUpper } from '../src/policy.js';

const fixture = JSON.parse(readFileSync('../tests/fixtures/resource_contract.json', 'utf8'));
...
test('nonfinite or coerced calibration bounds can never qualify', () => {
  for (const bound of [NaN, Infinity, -Infinity, 'not-a-number', '0.022', true, [], {}, null]) {
...
test('diagnostic and range contracts match strict portable Python artifacts', () => {
  for (const diagnostics of [{}, [], null, { ...fixture.artifact.diagnostics, unknown: 1 }]) {
    const raw = rawArtifact();
...
test('rounded limits remain exact and the joint bound rejects invalid denominators', () => {
  ...
  assert.throws(() => roundedLimit(Number.MAX_SAFE_INTEGER, 1000, 100), /inexact_resource_limit/);
...
test('coerced budgets follow the Python incomplete-feature guard', () => {
  ...
  for (const key of ['requested_compute_units', 'requested_loaded_accounts_bytes']) {
    for (const value of [true, '1000', -1, 1.5, NaN]) {
      const features = { ...example.features, [key]: value } as Features;
      assert.equal(
        predictResources(artifact, features, example.context, BigInt(example.current_slot)).reason,
        'incomplete_features',
      );
    }
  }
...
test('JavaScript prototype names are unknown patterns, never implicit profiles', () => {
  ...
  for (const pattern_id of ['toString', '__proto__', 'constructor']) {
    const result = predictResources(
      artifact, { ...example.features, pattern_id }, example.context, BigInt(example.current_slot),
    );
...
test('exact slot types and evidence window boundaries stay strict', () => {
  for (const value of [400, 400n, true, '0400', '+400', '4e2', '-1', (2n ** 64n).toString()]) {
